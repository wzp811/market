			...
			<div class="search">
				<form method="get" action="/jsp/user.do">
					...
					<span>订单编码：</span> <input name="queryname" class="input-text"
						...
						href="/page/order/add.jsp">添加订单</a>
				</form>
			</div>
			<!--用户-->
				...
				<c:forEach items="${pa.items }" var="u" varStatus="line">
					<tr>
						...
						<td>
							...
							<span><a class="deleteUser" href="/order/remove?id=${u.id}" onclick="return confirm('确定删除订单<${u.code}>吗？')"><img src="/images/schu.png" alt="删除" title="删除" /></a></span>
						</td>
					</tr>
				</c:forEach>
			</table>
...
<script type="text/javascript">
	...
	if("${pa.currPage==1}" != "true"){
		$(".page-bar span:last a:eq(0)").attr("href", "/order/query?currPage=1&pageSize=${pa.pageSize}");
		$(".page-bar span:last a:eq(1)").attr("href", "/order/query?currPage=${pa.currPage-1}&pageSize=${pa.pageSize}");
	}

	//判断, 当前是否是最后页
	if("${pa.currPage==pa.totalPage}" != "true"){
		$(".page-bar span:last a:eq(2)").attr("href", "/order/query?currPage=${pa.currPage+1}&pageSize=${pa.pageSize}");
		$(".page-bar span:last a:eq(3)").attr("href", "/order/query?currPage=${pa.totalPage}&pageSize=${pa.pageSize}");
	}
...
<script type="text/javascript">
	...
	$(".page-bar [name='pageNum']").on("change", function(){
		...
		//跳转
		location = "/order/query?pageSize=${pa.pageSize}&currPage="+pageNum;
	})
	...
	$(".page-bar [name='pageCode']").next().on("click", function(){
		...
		//跳转
		location = "/order/query?pageSize=${pa.pageSize}&currPage="+pageCode;
	})
</script>
...
<script type="text/javascript">
	$(".page-bar [name='pageSize']").on("change", function(){
		...
		//跳转
		location = "/order/query?currPage=${pa.currPage}&pageSize="+size;
